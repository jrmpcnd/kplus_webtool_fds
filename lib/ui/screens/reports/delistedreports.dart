...
class DeListedReports extends StatefulWidget {
  const DeListedReports({Key? key}) : super(key: key);

  @override
  State<DeListedReports> createState() => _DeListedReportsState();
}

class _DeListedReportsState extends State<DeListedReports> {
  // Timer? _timer;
  //
  // void _startTimer() {
        ...
        builder: (BuildContext context, Widget? child) {
          return Theme(
              data: ThemeData.light().copyWith(
                primaryColor: const Color(0xff941c1b),
                hintColor: const Color(0xff941c1b),
                colorScheme: const ColorScheme.light(
                  primary: const Color(0xff941c1b),
                ),
                buttonTheme: const ButtonThemeData(textTheme: ButtonTextTheme.primary),
                textTheme: const TextTheme(
                  bodySmall: TextStyle(fontSize: 10, fontFamily: 'RobotoThin', color: Color(0xff941c1b)),
                  titleMedium: TextStyle(fontSize: 50, fontFamily: 'RobotoThin', color: Color(0xff941c1b)),
                ),
              ),
              child: child!);
        });
  ...
  Future<void> generateFile(BuildContext context) async {
    final String url = 'https://dev-chatbot-services.fortress-asya.com:19002/api/public/v1/ngo/test/reports/download/delisted?file_type=$selectedFileType&area=$selectedArea&unit=$selectedUnit&center_name=$selectedCenter&date_from=$selectedFrom&date_to=$selectedTo';
    try {
...
//for sorting function
  ...
  void fetchData(int page, int perPage) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('https://dev-chatbot-services.fortress-asya.com:19002/api/public/v1/ngo/test/get/all/blacklisted/clients?page=$page&perPage=$_selectedItem'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['retCode'] == '200' && jsonData['message'] == 'Blacklisted Clients Found') {
  ...
  void initState() {
    ...
    fetchRows();
    updateUrl('/Reports/Delisted_Reports');
  ...
  void fetchDistinctData(int page, int perPage, String area) async {
    ...
    try {
      final token = getToken();
      final response = await http.get(
        Uri.parse('https://dev-chatbot-services.fortress-asya.com:19002/api/public/v1/ngo/test/get/all/delisted/clients/reports?page=$page&perPage=$_selectedItem&area=$selectedArea&unit=$selectedUnit&center_name=$selectedCenter&date_from=$selectedFrom&date_to=$selectedTo'),
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            const HeaderBar(screenText: 'DELISTED CLIENTS'),
            Container(
                ...
                Container(
                  ...
                  child: Responsive(
                      desktop: Row(
                        ...
                        children: [areaFields(), unitField(), centerField(), dateFrom(), dateTo()],
                      ),
                      tablet: Wrap(
                        runSpacing: 10,
                        children: [
                          Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [areaFields(), unitField(), centerField()],
                          ),
                          Row(
        ...
        SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintText: selectedCenter.isEmpty ? 'DATE TO' : selectedCenter,
              hintStyle: const TextStyle(fontFamily: 'RobotoThin', fontSize: 11, color: Colors.black54),
            ),
            readOnly: true,
          ),
