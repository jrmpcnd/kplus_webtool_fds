...
typedef FlutterVoidCallback = void Function();
...
class _BatchTopUpState extends State<BatchTopUp> with SingleTickerProviderStateMixin {
  ...
  Future<void> topUp(Blob? file) async {
    ...
    reader.onLoadEnd.listen((e) async {
      ...
      final topUpProvider = Provider.of<TopUpProvider>(context, listen: false);
      await topUpProvider.fetchBatchTopUpResults(fileBytes, selectedFileName);
  ...
  void _downloadFile() async {
    ...
    final response = await DownloadTopUpAPI.downloadTopUpFile();
          ...
          onPressed: () {
            areButtonsEnabled
                ? uploadStatus != 'SUCCESS'
                    ? showBatchTopUpConfirmAlertDialog(context)
...
// Reusable Header Widget
  ...
  ///TOP UP FUNCTIONS
  void showBatchTopUpConfirmAlertDialog(BuildContext context, {FlutterVoidCallback? closeDialog}) {
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => AlertDialogWidget(
        titleText: "Batch Top Up Confirmation",
