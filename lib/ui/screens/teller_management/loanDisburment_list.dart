...
class LoanDisbursementClients extends StatefulWidget {
  const LoanDisbursementClients({Key? key}) : super(key: key);

  @override
  State<LoanDisbursementClients> createState() => _LoanDisbursementClientsState();
}

class _LoanDisbursementClientsState extends State<LoanDisbursementClients> {
...
//for sorting function
  ...
  Future<void> fetchDistinct() async {
    try {
      final token = getToken();
      final response = await http.get(
        Uri.parse('${UrlGetter.getURL()}/loan/test/get/all/clients?page=1&perPage=10&batch_loan_disbursement_file_id=1&batch_loan_disbursement_status=&topup_reference_id='),
        headers: {
          'Authorization': 'Bearer $token',
        },
      );
  ...
  void fetchData(int page, int perPage, String? batchTopupFileId) async {
    ...
    try {
      final token = getToken();
      final url = Uri.parse('${UrlGetter.getURL()}/loan/test/get/all/clients?page=1&perPage=10&batch_loan_disbursement_file_id=1&batch_loan_disbursement_status=&topup_reference_id=');
      fileNameController.text = selectedFileName!;
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('Top Up Clients List', style: TextStyle(fontWeight: FontWeight.bold),),
            Container(
