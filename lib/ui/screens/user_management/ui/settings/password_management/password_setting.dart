...
class _PasswordManagementState extends State<PasswordManagement> {
  ...
  final TapGestureRecognizer _tapGestureRecognizer = TapGestureRecognizer()
    ..onTap = () {
      CustomToast.show(navigatorKey.currentContext!, 'No description yet.');
    };
  ...
  // Future<void> initializePasswordInfo() async {
  //   TimerProvider passwordTimeProvider = Provider.of<TimerProvider>(context, listen: false);
  //   await passwordTimeProvider.fetchPasswordTime();
  //   timeoutNumberController.text = passwordTimeProvider.passwordDuration;
  //   timeoutFormatController.text = passwordTimeProvider.passwordFormat;
  //
  //   debugPrint('Fetched Password Time: ${passwordTimeProvider.passwordDuration}');
  //   debugPrint('Fetched Password Format: ${passwordTimeProvider.passwordFormat}');
  // }
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                const Spacer(),
                MyButton.buttonWithLabel(context, () => submitUpdatePassword(), 'Save Changes', Icons.save_outlined, AppColors.ngoColor),
              ],
            ),
          )
        ],
  ...
  Widget passwordPhraseBody() {
    return Column(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
          "Automatically expires new user's password.",
        ...
        RichText(
          text: TextSpan(
            children: [
              const TextSpan(
                text: 'The set password of the new and existing accounts will expires within the specified limit. ',
        ...
        const Text(
          "The changes will only take effect to the subsequent password reset.",
          style: TextStyles.dataTextStyle,
        ),
      ],
    );
  ...
  void submitUpdatePassword() async {
    // Retrieve data from controllers
    int passwordTime = int.parse(timeoutNumberController.text);
    String passwordFormat = timeoutFormatController.text;

    debugPrint('Fetched Password Format: $passwordFormat');
    debugPrint('Fetched Password Duration: ${passwordTime.toString()}');
      ...
      builder: (context) => const Center(
        child: Wrap(
          ...
          children: [
            ...
            Text(
              'Saving new password expiration...',
    ...
    final response = await SettingsAPI.updatePasswordTime(
      passwordTime,
      passwordFormat,
    );
  ...
  Future<void> showSuccessFailAlertDialog({required bool isSuccess}) async {
    showDialog(
      ...
      builder: (context) => AlertDialogWidget(
        titleText: isSuccess ? "Password Expiration Parameter Updated" : "Failed to Update",
        contentText: isSuccess ? "The password expiration was updated successfully." : "Sorry! The password's parameter was not updated.",
