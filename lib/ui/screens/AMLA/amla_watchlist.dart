...
import 'package:mfi_whitelist_admin_portal/core/mfi_whitelist_api/amla/amla_api.dart';
...
class AMLAWatchlist extends StatefulWidget {
  const AMLAWatchlist({Key? key}) : super(key: key);

  @override
  State<AMLAWatchlist> createState() => _AMLAWatchlistState();
}

class _AMLAWatchlistState extends State<AMLAWatchlist> {
  ...
  void fetchData(int page, int perPage) async {
    ...
    try {
      final token = getToken();
      final url = Uri.parse('${UrlGetter.getURL()}/amla/test/get/all/watchlisted?page=$page&perPage=$_selectedItem&file_name=$selectedFileName');
  ...
  List<DataRow> getPaginatedRows() {
        ...
        Align(
          ...
          child: rowData.isEditing && editableFields.contains(key)
              ? key == 'birthday'
                  ? GestureDetector(
                      onTap: () {
                        showModalDatePicker(context, rowData.controllers[key]!, DateTime.now(), 'Select Birthdate', false, () {});
                        // setState(() {
    ...
    Color _getWatchlistClassificationColor(String? status) {
      switch (status?.toUpperCase()) {
        ...
        case 'WATCHLISTED':
          return Colors.blue; // Green color for Approved status
        default:
    ...
    return List<DataRow>.generate(apiData.length, (index) {
      ...
      //final statusColor = _getWatchlistClassificationColor(rowData.data['watchlistedType']).withOpacity(0.2);
      ...
      return DataRow(
        ...
        cells: [
            ...
            Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: rowData.data['watchlistedType'] != 'N/A' && rowData.data['watchlistedType'] != '' && rowData.data['watchlistedType'] != null ? _getWatchlistClassificationColor(rowData.data['watchlistedType']).withOpacity(0.2) : Colors.transparent,
                  boxShadow: rowData.data['watchlistedType'] != 'N/A' && rowData.data['watchlistedType'] != '' && rowData.data['watchlistedType'] != null
                      ? [
                          BoxShadow(
                            color: rowData.data['watchlistedType'] != 'N/A' && rowData.data['watchlistedType'] != '' && rowData.data['watchlistedType'] != null ? _getWatchlistClassificationColor(rowData.data['watchlistedType']).withOpacity(0.2) : Colors.transparent,
                            spreadRadius: 2,
                ...
                child: Center(
                  child: Text(
                    ...
                    style: TextStyle(
                      color: rowData.data['watchlistedType'] != 'N/A' ? _getWatchlistClassificationColor(rowData.data['watchlistedType']) : Colors.transparent,
          ...
          // buildDataCell('memberClassification', rowData),
    ...
    return Container(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            const HeaderBar(screenText: 'AMLA WATCHLIST CLIENTS'),
            Container(
            ...
            PaginationControls(currentPage: displayPage, totalPages: totalPages, totalRecords: totalRecords, onPreviousPage: previousPage, onNextPage: nextPage, title: 'Watchlist'),
            // Row(
            ...
            //         ),
            //         Text(
            //           'Total Number of Watchlist: $totalRecords',
            //           style: const TextStyle(fontSize: 10, letterSpacing: 1, fontWeight: FontWeight.w500, color: Colors.black54),
            //         ),
            //       ],
            //     ),
  ...
  void submitUpdateClientForm(RowData rowData, int index) async {
    ...
    ClientDataModel clientData = ClientDataModel(
      ...
      isWatchlisted: rowData.data['isWatchlisted'],
    );
    ...
    // Call the API
    final response = await UpdateAMLAClientInfoAPI().updateAMLAClient(clientData);
  ...
  void submitDelistClient(String cid, int index) async {
    ...
    // Call the API
    final response = await DelistAMLAClientAPI().delistAMLAClient(cid);
    Navigator.pop(navigatorKey.currentContext!); // Dismiss the loading dialog
  ...
  void _showDelistAlertDialog(BuildContext context, String cid, int index) {
    showDialog(
      ...
      builder: (context) => AlertDialogWidget(
        ...
        contentText: "You will be de-listing a watchlist client. Proceed with caution.",
  ...
  void _showConfirmUpdateAlertDialog(BuildContext context, dynamic cid, int index) {
    showDialog(
      ...
      builder: (context) => AlertDialogWidget(
        titleText: "Confirmation",
        contentText: "You will be updating a client's info tagged in watchlist. Proceed with caution.",
