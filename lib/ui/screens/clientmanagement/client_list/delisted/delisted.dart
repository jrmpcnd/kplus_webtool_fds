...
class MFIDelistedClients extends StatefulWidget {
  const MFIDelistedClients({Key? key}) : super(key: key);

  @override
  State<MFIDelistedClients> createState() => _MFIDelistedClientsState();
}

class _MFIDelistedClientsState extends State<MFIDelistedClients> {
  ...
  void initState() {
    ...
    updateUrl('/Access/Client_List/Delisted_Clients');
  }
  ...
  void fetchData(int page, int perPage) async {
    ...
    try {
      final token = getToken();
      final url = Uri.parse('${UrlGetter.getURL()}/clients/test/get/all/delisted');
