...
class PendingTopupClientPerFiles extends StatefulWidget {
  final String fileName;
  final int batchID;
  final String status;
  final Function() onSuccessSubmission;
  const PendingTopupClientPerFiles({Key? key, required this.fileName, required this.batchID, required this.status, required this.onSuccessSubmission}) : super(key: key);

  @override
  State<PendingTopupClientPerFiles> createState() => _PendingTopupClientPerFilesState();
}

class _PendingTopupClientPerFilesState extends State<PendingTopupClientPerFiles> {
  ...
  void initState() {
    ...
    //trxController = TextEditingController(text: widget.trx.toString());
    ...
    print(batchUploadIDController.text);
  }
...
//   }
  ...
  Uri generateUrl(int page, int perPage, int batchUploadID, String status) {
    ...
    switch (status.toUpperCase()) {
      case 'UPLOADED':
        return Uri.parse('$baseUrl/topup/test/get/all/clients?page=$page&perPage=$perPage&batch_topup_file_id=$batchUploadID&batch_topup_status=&topup_reference_id=');
      case 'APPROVED':
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['retCode'] == '200') {
          ...
          var batchUploadID = jsonData['data'][0]['batchTopupFileId'];
