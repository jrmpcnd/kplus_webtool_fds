...
class FetchClientListAPIs {
  ...
  //SEARCH ON UPLOADED FILES
  static Future<List<AllTopUpUploadedFilesData>> searchAllUploadedFiles(String filename) async {
    final response = await http.get(
      Uri.parse('${UrlGetter.getURL()}/topup/test/get/all/uploaded?file_name=$filename'),
      headers: {'Authorization': 'Bearer $token'},
    );
...
class FetchClientDataApiServices {
  final String baseUrl;

  FetchClientDataApiServices(this.baseUrl);
...
class ClientService {
  static Future<List<ClientTopUpDataModel>> fetchClients() async {
    final url = '${UrlGetter.getURL()}/topup/test/get/all/clients';
    final response = await http.get(Uri.parse(url));

    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      final List<dynamic> clientData = data['data'] ?? [];
      return clientData.map((json) => ClientTopUpDataModel.fromJson(json)).toList();
    } else {
